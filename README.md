...
| 14 | `14_accreditation_bodies.sql` | `accreditation_bodies` and `institution_accreditations` tables (CUE, TVETA, KNEC, NCK, MPDC, etc. and their links to institutions), plus `institutions.accreditation_status`. Tables already existed in Supabase — this migration documents the applied schema. |
| 15 | `15_major_sponsors.sql` | `major_sponsors` table — the site-wide "Powered by" sponsor (distinct from placement-based `sponsor_ads`). Only one row can be `is_active` at a time, enforced by a partial unique index. |
| 16 | `16_share_analytics.sql` | `search_analytics`, `program_views`, `shared_searches` tables for the share/analytics module. All three are `service_role`-only (RLS + explicit policies), written/read exclusively via the Express API, same convention as payments. |
...
| 18 | `18_advertisers_user_id.sql` | Adds `user_id` (FK to `auth.users`), `balance`, and `total_spent` to `advertisers` — the table itself was added directly via dashboard without an auth link, so the advertiser portal's Supabase-Auth login had no way to find "my" advertiser record. |
| 19 | `19_ad_payments_advertiser_id.sql` | Adds `advertiser_id` (FK to `advertisers`) to `ad_payments`, so wallet top-ups not tied to a campaign (`campaign_id` nullable) are still queryable back to the advertiser that made them. |
| 20 | `20_campaign_clicks.sql` | New `campaign_clicks` table for the `ad_campaigns` system — mirrors `ad_clicks`' shape but references `ad_campaigns(id)` instead of `sponsor_ads(id)`, since a single FK column can't point at two parent tables and repointing `ad_clicks` would break the live sponsor-ads click tracking. |
...
| 22 | `22_referrals_referrer_device_id.sql` | Adds `referrer_device_id` to `referrals`, so a completed referral can pay out points to the referring device's `gamification_points` ledger. Pre-existing rows backfill to `NULL` and are skipped by the payout path, not retroactively credited. |
| 23 | `23_gamification_points_badge_action_type.sql` | Widens `gamification_points_action_type_check` to allow `action_type = 'badge'`, so badge bonus points get their own ledger entries. Looks up the existing constraint name dynamically since `gamification_points` was never created via a tracked migration (see "Known drift"). `'badge'` is intentionally excluded from the backend's client-facing `ACTION_POINTS` map — only the server-side badge-award path can insert this action type. |
